% Implements a classic control law based on the Damped Least Squares inverse and an Euclidean error.
%
% Usage: controller = DQ_DampedLeastSquaresSetpointController(robot), where
% robot is a DQ_Kinematics object.
%
% DQ_DampedLeastSquaresSetpointController Methods:
...
%   set_damping_coeficient - Sets the damping coeficient.
%
...
classdef DQ_DampedLeastSquaresSetpointController < DQ_KinematicSetpointController
    properties (Access=protected)
        damping_coeficient = 0;
    end
    
    methods
        function controller = DQ_DampedLeastSquaresSetpointController(robot)
            controller = controller@DQ_KinematicSetpointController(robot);
        end
        
        function set_damping_coeficient(obj,damping_coeficient)
            obj.damping_coeficient = damping_coeficient;
        end
        ...
        function u = compute_control_signal(controller, q, task_reference)
            ...
            if controller.is_set()
                ...
                u = (J'*J+controller.damping_coeficient*eye(length(q)))\(J'*controller.gain*task_error);
